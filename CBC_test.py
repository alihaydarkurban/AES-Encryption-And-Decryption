...
import CBC as cbc       # Import CBC to test it
...
def CBC_test(key, text):
    iv_enc_val = cbc.generate_initialization_vector()
    iv_dec_val = copy(iv_enc_val)

    cipher_text_arr, added_item_count = cbc.CBC_encryption(key, text, iv_enc_val)
    enc_str_arr = []
    for cipher_text in cipher_text_arr:
        enc_str_arr.append(aes.translate_hex_into_str(cipher_text))
    encrypt_text_str = cbc.merge_strings_without_remove_item(enc_str_arr)
    resolved_text_val = cbc.CBC_decryption(key, cipher_text_arr, iv_dec_val, added_item_count)
...
if __name__ == '__main__':
    ...
    print("Example of AES encryption and decryption with version of CBC")
    ...
    for test_num in range(2):
        ...
        CBC_test(KEYS[test_num], TEXTS[test_num])
        print_separator_long()